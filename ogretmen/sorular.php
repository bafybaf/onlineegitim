...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        question_answer((int) $u['id'], (int) post('qid'), post('answer'));
        flash_ok('Yanıt gönderildi.');
        redirect('ogretmen/sorular.php?id=' . (int) post('qid') . '&durum=hepsi');
    } catch (Throwable $e) {
        flash_error($e->getMessage());
        redirect('ogretmen/sorular.php?id=' . (int) post('qid'));
    }
}
$sql = "SELECT q.*, u.name sname, u.email semail, g.name gname
...
<p class="mb-4 text-sm text-muted">Öğrencilerinizin ders soruları. Yanıt öğrenci paneline düşer.</p>
<div class="mb-4 flex flex-wrap gap-2">
  <a class="btn-outline text-sm <?= $durum === 'bekleyen' ? 'border-navy' : '' ?>" href="<?= e(url('ogretmen/sorular.php?durum=bekleyen')) ?>">Bekleyen<?= $pending ? ' (' . $pending . ')' : '' ?></a>
  <a class="btn-outline text-sm <?= $durum === 'cevapli' ? 'border-navy' : '' ?>" href="<?= e(url('ogretmen/sorular.php?durum=cevapli')) ?>">Cevaplanan</a>
  <a class="btn-outline text-sm <?= $durum === 'hepsi' ? 'border-navy' : '' ?>" href="<?= e(url('ogretmen/sorular.php?durum=hepsi')) ?>">Tümü</a>
</div>
...
<?php foreach ($rows as $q):
  ...
  <article class="card mb-4 p-5 <?= $on ? 'border-navy' : '' ?>" id="q-<?= (int) $q['id'] ?>">
    <div class="flex flex-wrap items-start justify-between gap-2">
      <div>
        <p class="text-xs font-extrabold uppercase text-navy"><?= e((string) ($q['gname'] ?: 'Ders')) ?></p>
        <h2 class="font-display mt-1 text-2xl"><?= e((string) $q['sname']) ?></h2>
        <p class="text-sm text-muted"><?= e((string) $q['semail']) ?> · <?= e((string) $q['created_at']) ?></p>
      </div>
